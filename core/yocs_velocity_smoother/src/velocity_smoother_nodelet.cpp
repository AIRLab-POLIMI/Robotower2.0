...
void VelocitySmoother::reconfigCB(yocs_velocity_smoother::paramsConfig &config, uint32_t level)
{
  ROS_INFO("Reconfigure request : %f %f %f %f %f",
           config.speed_lim_vx, config.speed_lim_w, config.accel_lim_vx, config.accel_lim_w, config.decel_factor_x);
  ...
  decel_factor_x = config.decel_factor_x;
  decel_factor_y = config.decel_factor_y;
  decel_factor_w = config.decel_factor_w;
  
  decel_lim_vx  = decel_factor_x*accel_lim_vx;
  decel_lim_vy  = decel_factor_y*accel_lim_vy;
...
void VelocitySmoother::spin()
{
  ...
  while (! shutdown_req && ros::ok())
  {
        ...
        (target_vel.angular.z != last_cmd_vel.angular.z))
    {
      ...
      double x_inc, w_inc, y_inc, max_y_inc, max_x_inc, max_w_inc;

      x_inc = target_vel.linear.x - last_cmd_vel.linear.x;
      ...
      if ((robot_feedback == ODOMETRY) && (current_vel.linear.x * target_vel.linear.x < 0.0)){
          ...
          max_x_inc = decel_lim_vx * period;
      }else {
          max_x_inc = ((x_inc * target_vel.linear.x > 0.0) ? accel_lim_vx : decel_lim_vx) * period;
      }

      y_inc = target_vel.linear.y - last_cmd_vel.linear.y;
      
      if ((robot_feedback == ODOMETRY) && (current_vel.linear.y * target_vel.linear.y < 0.0)){
          // countermarch (on robots with significant inertia; requires odometry
          // feedback to be detected)
          max_y_inc = decel_lim_vy * period;
      }else {
          max_y_inc = ((y_inc * target_vel.linear.y > 0.0) ? accel_lim_vy : decel_lim_vy) * period;
      }
      ...
      double MA = sqrt(    x_inc *     x_inc + y_inc * y_inc +    w_inc *     w_inc);
      double MB = sqrt(max_x_inc * max_x_inc + max_y_inc * max_y_inc + max_w_inc * max_w_inc);

      double Ax  = std::abs(x_inc) / MA;
      double Ay  = std::abs(y_inc) / MA;
      double Aw  = std::abs(w_inc) / MA;

      double Bx  = max_x_inc / MB;
      double By  = max_y_inc / MB;
      double Bw  = max_w_inc / MB;
      double theta1 = atan2(Bw, Bx) - atan2(Aw, Ax);
      double theta2 = atan2(Bw, By) - atan2(Aw, Ay);

      if (theta1 < 0) {
        // overconstrain linear velocity
        max_x_inc = (max_w_inc * std::abs(x_inc)) / std::abs(w_inc);
      } else {
        // overconstrain angular velocity
        max_w_inc = (max_x_inc * std::abs(w_inc)) / std::abs(x_inc);
      }
      
      if (theta2 < 0) {
        // overconstrain linear velocity
        max_y_inc = (max_y_inc * std::abs(y_inc)) / std::abs(w_inc);
      } else {
        // overconstrain angular velocity
        max_w_inc = (max_x_inc * std::abs(w_inc)) / std::abs(x_inc);
      }
      ...
      if (std::abs(x_inc) > max_x_inc) {
        // we must limit linear velocity
        cmd_vel->linear.x = last_cmd_vel.linear.x + sign(x_inc) * max_x_inc;
      }

      if (std::abs(y_inc) > max_y_inc) {
        // we must limit linear velocity
        cmd_vel->linear.y = last_cmd_vel.linear.y + sign(y_inc) * max_y_inc;
      }
...
bool VelocitySmoother::init(ros::NodeHandle& nh)
{
  ...
  nh.param("decel_factor",   decel_factor_x,   1.0);
  ...
  if ((nh.getParam("accel_lim_vy", accel_lim_vy) == false) ||
