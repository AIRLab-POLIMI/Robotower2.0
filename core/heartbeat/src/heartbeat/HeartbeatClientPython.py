...
class HeartbeatClientPy:
	def __init__(self):
		...
		try:
			...
			rospy.loginfo( "Service register_node call")
		...
		try:
			...
			rospy.loginfo( "Service unregister_node call" )
		except rospy.ServiceException, e:
			rospy.loginfo( "Service call failed: %s,"%e )
		rospy.wait_for_service('/heartbeat/register_node')
		...
		try:
			...
			rospy.loginfo( "Service set_state call")
		except rospy.ServiceException, e:
			rospy.loginfo( "Service call failed: %s,"%e	)
	...
	def start(self, name):
		...
		if (self.register_node(self.node_name, 1)):
			rospy.loginfo( "Node heartbeat sucessfully registered")
		else:
			rospy.loginfo( "Node heartbeat failed to register")

	def stop(self, name):	
		if (not(self.unregister_node(name))):
			rospy.loginfo( "Heartbeat unregister RPC failed")		
		else:
			rospy.loginfo( "Node unregistered from Heartbeat")
