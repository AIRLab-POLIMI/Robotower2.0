...
#define FEEDBACK_COM_THD 1000
#define FEEDBACK_LED 1000
...
void ledCallback(const game_manager::ChangeLEDs &msg){
  bool reset_array[] = {0,0,0,0};
  if (msg.id == TOWER_ID){
    updateChargeLEDs(reset_array /*msg.charge_leds*/);
...
void updateChargingLEDs(){
  for(int i=0;i<N_CHARGE_LEDS;i++){
    digitalWrite(charge_LEDs[i],led_status[i]);
  }
}
...
void loop() {
  ...
  if (button_state){
    if ((millis() - starting_time) > FEEDBACK_LED){
        index_led = -1;
        for(int j=0; j<current_led; j++){
          led_status[j]=1;
        }
        if(current_led < 3){
          if (led_status[current_led]==0){
            // Get first led light off
            index_led = current_led;
            // index_led = j;
            // Light up the first light off
            led_status[index_led]=1;
            updateChargingLEDs();
            ...
            // Exit from for loop
            // break;
            //digitalWrite(charge_LEDs[index_led],HIGH);
          }
        }
        //}
        if (index_led == -1){
          // All leds were on
          index_led = 4;
          changeStatusLED(GREEN_COLOR);
          //status_color[0] = GREEN_COLOR[0];
          //status_color[1] = GREEN_COLOR[1];
          //status_color = {0,1};
          //digitalWrite(charge_LEDs[index],HIGH);
        }
    }
  }
  else{
  ...
  if ((millis() - feedback_com_msg_timer) > FEEDBACK_COM_THD){
    //Serial.println("Publishing..");
    feed_msg.header.stamp = nh.now();
    feed_msg.id = TOWER_ID;
    // copying arrays
    for(int i=0; i<3; i++){
      feed_msg.charge_leds[i] = led_status[i];
    }
    for(int h=0; h<2; h++){
      feed_msg.status_led_color[h] = status_color[h];
    }
    // copy_vector(feed_msg.charge_leds, led_status, 3);
