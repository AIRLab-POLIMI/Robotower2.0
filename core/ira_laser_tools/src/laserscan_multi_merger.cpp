...
void LaserscanMerger::laserscan_topic_parser()
{
    ...
    for (ros::master::V_TopicInfo::iterator it = topics.begin() ; it != topics.end(); it++) {
        ...
        std::cout << "Topic : " << it - topics.begin() << ": " << info.name << " -> " << info.datatype <<       std::endl;
    }
    */

    /* THIS MUST BE UNCOMMENTED LATER WHEN FIXING THE PROBLEM. THIS IS THE GENERAL WAY TO CHECK THE TOPICS.
	for(int i=0;i<tokens.size();++i){
...
int main(int argc, char** argv)
{
    ...
    ROS_INFO_STREAM("Spinning merger...");
	ros::spin();
