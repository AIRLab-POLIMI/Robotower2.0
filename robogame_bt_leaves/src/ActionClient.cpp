...
int main (int argc, char **argv)
{
	...
	// create the action client
  	...
  	actionlib::SimpleActionClient<robogame_bt_leaves::BTAction> ac("action", true);
	...
	bool isRunning = false;
 	...
 	while(command!=3){
		...
		ROS_INFO("Send a command by pressing followed by enter:\n 1:start the action | 2:stop the action | 3:exit the program");
		std::cin >> command;
