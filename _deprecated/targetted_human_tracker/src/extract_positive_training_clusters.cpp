...
// Custom messages
#include <targetted_human_tracker/cluster_features.h>
#include <targetted_human_tracker/laser_processor.h>
...
*
* Reads in a rosbag, finds all scan clusters lying within user-specified
* bounding box (or alternatively min/max angles),
    ...
    */
    ExtractPositiveTrainingClusters() {
        ...
        if (!nh_private.getParam("x_min", x_min_) or
            ...
            ROS_INFO("Couldn't get bounding box for positive clusters. Assuming "
                    "you've specified a min/max scan angle and a max distance "
                    "instead.");
        ...
        if (!nh_private.getParam("min_angle", min_angle_) or
            ...
            if (use_bounding_box_) {
              ROS_INFO("Couldn't get min/max scan angle for positive clusters. "
                      "Assuming you've specified a bounding box instead.");
            } else {
              ROS_ERROR("Couldn't get bounding box or scan min/max angle for "
                        "positive clusters");
            }
        }
    ...
    /**
    * @brief Extract the positive clusters and record their position with a
    * leg_cluster_positions message
    */
    void extract() {
...
int main(int argc, char **argv) {
    ...
    eptc.extract();
    ROS_INFO("Finished successfully! (you still have press ctrl+c to terminate "
            "if you ran from a launch file)");
