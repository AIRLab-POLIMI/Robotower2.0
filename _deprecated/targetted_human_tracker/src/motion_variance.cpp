#include <targetted_human_tracker/motion_variance.h>
...
void MotionVariance::initRosComunication(){
    ...
    variance_pub_ = nodeHandle().advertise<targetted_human_tracker::TrackVariance>("/track_variance", 10);
    player_track_sub_ = nodeHandle().subscribe("/players_tracked", 1, &MotionVariance::trackCallback, this);
}

void MotionVariance::convertToOdomFrame(const targetted_human_tracker::PersonArray &msg){
    try{
...
void MotionVariance::trackCallback(const targetted_human_tracker::PersonArray &msg){
    convertToOdomFrame(msg);
}

void MotionVariance::blobDetectionCallback(const targetted_human_tracker::Blob &msg)
{
    if (!msg.observed) {
        return;
    }
