...
protected:
    ...
    void getFovVertices(const targetted_human_tracker::Blob &person, Vec2f &robot, Vec2f &fovLeft, Vec2f &fovRight);
    ...
    virtual void computeAssociations(const targetted_human_tracker::PersonArray &people, AssociationMatrix &association, IntSet &peopleToAssociate);
    virtual void updateClouds(const targetted_human_tracker::PersonArray &people, AssociationMatrix &association, IntSet &peopleToAssociate);
    virtual void createNewClouds(const targetted_human_tracker::PersonArray &people, AssociationMatrix &association, IntSet &peopleToAssociate);

public:
    MultipleParticleFilter(int numParticles=500, int maxClouds=2);
    ~MultipleParticleFilter();

    void peopleDetectionCallback(const targetted_human_tracker::PersonArray &people);
    void blobDetectionCallback(const targetted_human_tracker::Blob &person);
