...
class Tracker:    
    ...
    def __init__(self):
        ...
        self.lda_blob = 0.5
        self.lda_player = 0.5
        self.last_seen_player_position_time = rospy.Time(0.0)
    ...
    def evidenceCallback(self, evidences):
        ...
        if len(self.objects_tracked) == 0:
            ...
            for idx, track in enumerate(self.objects_tracked):
                ...
                if matched_tracks[track] is None:
                    ...
                    #track.confidence = 0.8*track.confidence + 0.2*matched_tracks[track].confidence
                    track.times_seen += 1
                    track.last_seen = now
                    track.isPlayer = True
                ...
                if track.confidence < self.confidence_threshold_to_maintain_track:
                    ...
                    rospy.loginfo("deleting track due to low confidence of {}...".format(track.confidence))
                else:
                    ...
                    if cov > self.max_cov:
                        tracks_to_delete.add(track)
                        rospy.loginfo("deleting track due to large state covariance of {}...".format(cov))
            ...
            for ev in observations:
                if ev not in matched_tracks.values() and ev.confidence > 0.5: ## because the evidence may be too far away...
                    self.objects_tracked.append(ObjectTracked(ev.position.x, ev.position.y, 
                                                                now, 
                                                                ev.confidence, 
                                                                False,
        ...
        #         self.last_seen_player_position_time = rospy.get_rostime()
        #     else:
    ...
    def calcNewConfidence(self, position):
        ...
        return p_wrt_blob + p_wrt_current_player*np.exp(-self.last_seen_player_position_time.to_sec())

    def RBFKernel(self,x,b,lda):
        """lda is the parameter to adjust Kernel distance"""
