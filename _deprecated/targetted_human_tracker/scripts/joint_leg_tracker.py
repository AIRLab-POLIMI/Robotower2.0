...
# Custom messages
from targetted_human_tracker.msg import Person, PersonArray, Leg, LegArray 
...
class KalmanMultiTracker:    
    ...
    def publish_tracked_people(self, now):
            ...
            for person in self.objects_tracked:
                if person.is_person == True:
                    if self.publish_occluded or person.seen_in_current_scan: # Only publish people who have been seen in current scan, unless we want to publish occluded people
                        ...
                        people.append(copy.deepcopy(ps))
                        ...
                        # markers = createHuman(marker_id, new_person.pose, self.publish_people_frame, color=None)
                        # for m in markers:
                        #     m.color.a = (rospy.Duration(3) - (rospy.get_rostime() - person.last_seen)).to_sec()/rospy.Duration(3).to_sec() + 0.1
        ...
        self.polar_grid.publish_marker(people,now)
