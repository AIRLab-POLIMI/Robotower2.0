...
ParticleFilter::ParticleFilter(int num_particles) : num_particles(num_particles) {
    ...
    pub_centers = nh.advertise<geometry_msgs::PoseArray>("/pf_cloud_centers", 10);
   

    legs_sub = nh.subscribe("filtered_detected_leg_clusters", 10, &ParticleFilter::legCallback, this);
    sub_occupancy_grid = nh.subscribe("test_local_map", 10, &ParticleFilter::occupancyGridCallback, this);
    ros::service::waitForService("get_human_leg_context", -1);
    leg_context_service_client = nh.serviceClient<player_tracker::LegProbability>("get_human_leg_context");
    //personDetectedSub = nh.subscribe("/people_tracked", 10, &ParticleFilter::peopleDetectedCallback, this);
    //blobSub = nh.subscribe("/blob_detection", 10, &ParticleFilter::blobDetectedCallback, this);
    got_first_local_occ_grid = false;
    initParticles();
}
...
geometry_msgs::PointStamped ParticleFilter::getOverallEstimatedVariance(){
    double var_x = 0;
...
void ParticleFilter::computeWeights() {
    ...
    std::mt19937 gen(rd());
    std::normal_distribution<float> gauss(0.0f, 3);

    #pragma omp parallel for reduction(+:summation)
    for (int i = 0; i < particles.size(); i++) {


        auto particle_state = particles[i]->getState();
        
        if (got_first_local_occ_grid){
            auto grid_x = (unsigned int)((particle_state(0) - last_local_grid.info.origin.position.x) / last_local_grid.info.resolution);
            auto grid_y = (unsigned int)((particle_state(1) - last_local_grid.info.origin.position.y) / last_local_grid.info.resolution);

            double weight_from_grid = 100 * exp(last_local_grid.data[grid_x + last_local_grid.info.width * grid_y]);
            if (std::isinf(weight_from_grid)){
                weight_from_grid = 0;
            }else{
                weight_from_grid += gauss(gen);
                if (weight_from_grid < 0) weight_from_grid*=-1;
                //ROS_INFO_STREAM("Add weight: " << weight_from_grid);
            }

            particles[i]->setWeight(weight_from_grid);
        }else{
...
void ParticleFilter::computeWeights(Eigen::Matrix<float, 3, 1> &obs) {
    ...
    for (int i = 0; i < particles.size(); i++) {
        ...
        float angle_laser =  boost::numeric_cast<float> (atan2(obs(1),obs(0)));
        float angle_diff = fabs(angle_laser - obs(2));
        //ROS_INFO("Angle Laser: %f \t Cam Angle: %f \t Angle diff:  %f", angle_laser, obs(2), angle_diff);
        ...
        //ROS_INFO_STREAM("width: " << exp(100 * last_local_grid.data[grid_x + last_local_grid.info.width * grid_y]));

        auto weight_from_grid = 100 * exp(last_local_grid.data[grid_x + last_local_grid.info.width * grid_y]);
        if (std::isinf(weight_from_grid)){
            weight_from_grid = 0;
        }else{
            weight_from_grid += gauss(gen);
        ...
        float new_weight = (1/particles[i]->distance(obs)) + 1/exp(-angle_diff) + weight_from_grid;
...
void ParticleFilter::occupancyGridCallback(const nav_msgs::OccupancyGridPtr &msg){
    got_first_local_occ_grid = true;
    last_local_grid = *msg;
}

void ParticleFilter::legCallback(const player_tracker::LegArrayPtr &msg){
