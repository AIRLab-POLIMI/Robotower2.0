...
void Safety::SafetyEvaluator::velCallback(const geometry_msgs::Twist& vel){
    ...
    int current_angle_deg = current_angle_motion_ * (180 / M_PI);
    current_heading_sector_ = (current_angle_deg + 180) / 45;
...
void Safety::SafetyEvaluator::playerCallback(const std_msgs::Int32& angle){
    last_know_player_position_ = angle.data;
    current_player_sector_ = (last_know_player_position_ + 90) / 45;
    ROS_ERROR("Current player angle: %d", last_know_player_position_);
