...
void Behavior::Planner::updateDecisionVariables(){
    ...
    for(int i=0; i < num_towers_; i++ ){
        ...
        for (int j=0; j < goals_.size(); j++){
            float utility = -tower_player_distances_[i] * pow(blocking_factor_[i], bf_exponent_) - (leds_on_[i]*1000); //1000 is a larger weight associated with the number of leds.
