...
def createHuman(m_id, pose, target_frame, color=None):
    human = []
    m_id += 1
    human.append(createLabel(m_id, Marker.ADD, pose, target_frame))
    m_id += 1
...
def player_pose_callback(pose):
    ...
    p = Pose()
    p.position.x = pose.point.x
    p.position.y = pose.point.y
    p.position.z = pose.point.z
    player_marker = createHuman(1, p, pose.header.frame_id)


if __name__ == '__main__':
    rospy.init_node('human_marker_creator')
    sub = rospy.Subscriber('player_filtered', PointStamped, player_pose_callback)
    pub = rospy.Publisher('player_marker', Marker, queue_size=30)
    
    rate = rospy.Rate(10)
    
    while not rospy.is_shutdown():
        if player_marker is not None:
            for m in player_marker:
                pub.publish(m)
                player_marker = None
        rate.sleep()
