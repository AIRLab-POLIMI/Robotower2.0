...
public:
  ...
  */ 
    ...
    sync(laserScanSubscriber_, non_leg_clusters_sub_, 100)
  {
    ...
    nh_.param("cluster_dist_euclid", cluster_dist_euclid_, 0.3);
        ...
        { 
          for (int j = 0; j < width_; j++)
          {
            ...
            if (angle > scan.angle_min - scan.angle_increment/2.0 and angle < scan.angle_max + scan.angle_increment/2.0)
            {
              ...
              int closest_beam_idx = boost::numeric_cast<int>(round(angle/scan.angle_increment) + scan.ranges.size()/2);
              ROS_WARN("closest_beam_idx %d", closest_beam_idx);
              if (closest_beam_idx < 0)
                closest_beam_idx *= -1;
