...
void callback(const ImageConstPtr &depth, const ImageConstPtr &color,const GroundPlane::ConstPtr &gp, const CameraInfoConstPtr &info)
{
    ...
    img_depth_.convertTo(img_depth_,CV_32FC1, 1.0/255.0);
    
    resize(img_depth_, img_depth_, cv::Size(640, 480), 2.28, 2.28, cv::INTER_LANCZOS4);
    
    Matrix<double> matrix_depth(640,480);//info->width, info->height);
    for (int r = 0;r < 480;r++){
        for (int c = 0;c < 640;c++) {
	...
	ROS_INFO_STREAM("Is calculation necessary: " << vis);
...
void connectCallback(message_filters::Subscriber<CameraInfo> &sub_cam,
    ...
    if(!pub_message.getNumSubscribers()
        ...
        ROS_INFO("Upper Body Detector: No subscribers. Unsubscribing.");
        sub_cam.unsubscribe();
        sub_gp.unsubscribe();
        sub_col.unsubscribe();
        sub_dep.unsubscribe();
    } else {
        ROS_INFO("Upper Body Detector: New subscribers. Subscribing.");
        sub_cam.subscribe();
        sub_gp.subscribe();
...
void test_callback(const ImageConstPtr &depth, const ImageConstPtr &color,const GroundPlane::ConstPtr &gp, const CameraInfoConstPtr &info)
{
	ROS_INFO("test");
}
...
int main(int argc, char **argv)
{
    ...
    ROS_INFO_STREAM("Camera info topic: " << topic_camera_info);
    ...
    ROS_INFO_STREAM("Depth_image topic: " << topic_depth_image);

    // Printing queue size
    ROS_INFO("upper_body_detector: Queue size for synchronisation is set to: %i", queue_size);
